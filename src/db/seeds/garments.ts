...
async function main() {
    try {
        ...
        console.log('🧹 Clearing existing garments data...');
        await db.delete(garments);
        console.log('✅ Existing data cleared successfully');

        // Sample data from schema (exactly as provided)
        ...
        // Insert sample data
        console.log('📦 Inserting garments data...');
        ...
        if (insertedGarments.length === 10) {
            console.log('✅ Verification successful: All 10 garments inserted correctly');
            console.log('📊 Inserted garments:');
            insertedGarments.forEach((garment, index) => {
                console.log(`  ${index + 1}. ${garment.name} (ID: ${garment.id}, Price: $${garment.price})`);
            });
        } else {
            console.error(`❌ Verification failed: Expected 10 garments, found ${insertedGarments.length}`);
            process.exit(1);
        }
