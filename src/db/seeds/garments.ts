...
async function main() {
    ...
    // Sample garments data with exact specifications
    const sampleGarments = [
        {
            ...
            qualityRating: 9,
            history: 'The Anarkali dress originated from the Mughal era, named after the legendary courtesan Anarkali. This fusion version combines traditional silhouettes with modern fabric technology and contemporary cuts. The flowing design was favored by Mughal nobility and has evolved through centuries to become a staple in Indo-western fashion. Our version features hand-embroidered motifs inspired by Mughal architecture, representing the perfect marriage of historical elegance and modern sophistication.',
        ...
        },
        {
            ...
            qualityRating: 10,
            history: 'Indo-western gowns emerged in the 1960s as a fusion response to globalization and cultural exchange. This style combines the elegance of Western evening gowns with traditional Indian embellishments and draping techniques. The design philosophy draws from both Parisian haute couture and Indian craftsmanship, creating garments that celebrate cultural fusion while maintaining sophistication and grace for the modern cosmopolitan woman.',
    ...
    // Insert each garment individually to ensure database compatibility
    for (const garment of sampleGarments) {
        try {
            await db.insert(garments).values(garment);
            console.log(`✅ Successfully inserted: ${garment.name}`);
        } catch (error) {
            console.error(`❌ Failed to insert ${garment.name}:`, error);
            throw error;
        }
    }
    
    console.log('✅ Garments seeder completed successfully - All 10 Indian fusion garments inserted');
}
